...
const Pagination = ({ itemsPerPage, totalItems, currentPage, paginate }) => {
  ...
  const pageNumbers = Array.from({ length: Math.ceil(totalItems / itemsPerPage) })
  ...
  return (
    <div className='pagination'>
      ...
      <FontAwesomeIcon icon={faAnglesLeft} className='arrow' onClick={() => handleArrowClick('left')} />
      {pageNumbers.map((_, index) => (
        <button 
          className={`paginate-button ${activePage === index + 1 ? 'active' : ''}`}
          key={index} 
      ...
      <FontAwesomeIcon icon={faAnglesRight} className='arrow' onClick={() => handleArrowClick('right')}/>
    </div>
  );
}
